...
public:
	...
	void push(In value)
	{
		...
		pool.submit(0, [self, value]() {
			...
			{
				std::cerr << "Stage error: " << e.what() << std::endl;
			}
			catch (...)
			{
				std::cerr << "Unknown stage error\n";
			}
			});
	}
};
...
static inline void connect(std::shared_ptr<Stage<A, B>> a,
	std::shared_ptr<Stage<B, C>> b)
{
	a->set_next([b](B value) {
		b->push(value);
		});
}
